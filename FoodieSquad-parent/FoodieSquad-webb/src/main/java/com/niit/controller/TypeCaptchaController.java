...
public class TypeCaptchaController {
	...
	public ModelAndView test(@Param("mobile") String mobile,HttpSession session,HttpServletRequest request,HttpServletResponse response) {
		...
		if(c!=null){
        	...
        	System.out.println("String cellnum"+mobile);
        	if(oldnum.equals(mobile)) {
        		System.out.println("检测到相同电话号码，五分钟之内不再发送！");
        		try {
        			request.getRequestDispatcher("../login/verifyuser.do?userphone="+mobile).forward(request,response);
	    ...
	    mv.addObject("cellnum", mobile);
        System.out.println(" savephonenum(cellnum);");
        savephonenum(mobile,response);
