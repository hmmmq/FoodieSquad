...
if(num>0)
{
rs.push($(this).find(".cashName").text());
rs.push(num);
}
i++;
});
...
//window.location.href="/FOODIESQUAD/ChooseFoodServlet?menu="+rs+"&totalPrice="+totalp;   重要
});
});
</script>
</head>
...
<img src="${imgpth}backbutton.SVG" id="backbtn" class="backButtonSize"
     onclick="window.location.href='/FOODIESQUAD/CanteenServlet'"/><!-- 跳转修改 -->
</div>
<!--食堂信息-->
<div class="canteenInfoBox">
<div class="canteenInfoTextBox">
<p id="canteeninfo" class="canteenInfoText">${canteen.canteenName}</p>
<label for="canteeninfo" class="canteenTextReconmmendText">${canteen.canteenName}推荐：五谷鱼粉，麻辣香锅</label>
</div>
</div>
</div>
...
<!--菜肴名称-->
<td colspan="2" width="50%"><p  class="cashName">${dish.dishName}</p></td>
</tr>
<tr height="60%">
<!--菜肴单价和数量-->
